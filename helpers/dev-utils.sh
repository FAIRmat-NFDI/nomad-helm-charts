...
alias deploy-oasis='cd $(git rev-parse --show-toplevel)/charts/nomad-default && helm upgrade nomad-oasis . -f oasis-minikube-values.yaml -n nomad-oasis'
alias install-oasis='cd $(git rev-parse --show-toplevel)/charts/nomad-default && helm install nomad-oasis . -f oasis-minikube-values.yaml -n nomad-oasis --create-namespace'
