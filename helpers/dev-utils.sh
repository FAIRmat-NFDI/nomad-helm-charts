...
alias deploy-oasis='cd $(git rev-parse --show-toplevel)/charts/local-oasis && helm upgrade nomad-oasis . -f oasis-minikube-values.yaml -n nomad-oasis'
alias install-oasis='cd $(git rev-parse --show-toplevel)/charts/local-oasis && helm install nomad-oasis . -f oasis-minikube-values.yaml -n nomad-oasis --create-namespace'
